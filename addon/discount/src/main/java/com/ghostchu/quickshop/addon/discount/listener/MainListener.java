    ...
    Util.mainThreadRun(()->{
      switch(codeInstalled.applicableShop(purchaser, shop)) {
        ...
        case APPLICABLE_WITH_THRESHOLD ->
                quickshop.text().of(purchaser, "addon.discount.discount-code-applicable", code, quickshop.getEconomyManager().provider().format(BigDecimal.valueOf(codeInstalled.getThreshold()), shop.bukkitLocation().getWorld().getName())).send();
        case NOT_APPLICABLE ->
  ...
  public void onPurchase(final ShopPurchaseEvent event) {
    ...
    switch(codeInstalled.applicableShop(purchaser, shop)) {
      case APPLICABLE -> {
        ...
        quickshop.text().of(purchaser, "addon.discount.discount-code-applied-in-purchase", code, quickshop.getEconomyManager().provider().format(BigDecimal.valueOf(discounted), shop.bukkitLocation().getWorld().getName())).send();
      }
      case APPLICABLE_WITH_THRESHOLD -> {
        if(event.getTotal() < codeInstalled.getThreshold()) {
          quickshop.text().of(purchaser, "addon.discount.discount-code-under-threshold", quickshop.getEconomyManager().provider().format(BigDecimal.valueOf(codeInstalled.getThreshold()), shop.bukkitLocation().getWorld().getName())).send();
        } else {
          final double beforeDiscount = event.getTotal();
          event.setTotal(codeInstalled.apply(purchaser, event.getTotal()));
          final double discounted = beforeDiscount - event.getTotal();
