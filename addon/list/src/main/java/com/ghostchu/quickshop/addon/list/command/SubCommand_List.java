...
public class SubCommand_List implements CommandHandler<Player> {
  ...
  private void lookup(@NotNull final Player sender, @NotNull final UUID lookupUser, final int page) {
    ...
    for(final Shop shop : shops) {
      ...
      Component component = quickshop.text().of(sender, "addon.list.entry", counter, shopNameComponent, location.getWorld().getName(), location.getBlockX(), location.getBlockY(), location.getBlockZ(), shop.format(shop.bukkitLocation().getWorld().getName()), shop.getShopStackingAmount(), Util.getItemStackName(shop.getItem()), shopTypeComponent).forLocale();
