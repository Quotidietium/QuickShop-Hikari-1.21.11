# R52 A/B comparison: round52-baseline (e8924bea2, R51 done) vs round52
# (browse item-read collapse: live-prototype probes, single-meta search,
# uncloned group representative reads). Median ns/op and median B/op across all
...
a_ns, b_ns = load("round52-baseline", "medianNsPerOp"), load("round52", "medianNsPerOp")
a_b, b_b = load("round52-baseline", "medianBytesPerOp"), load("round52", "medianBytesPerOp")

common = sorted(set(a_ns) & set(b_ns))
print(f"shared cases: {len(common)}\n")

print("== B/op axis (primary; near-deterministic) ==")
print(f"{'case':40s} {'baseline B/op':>15s} {'R52 B/op':>15s} {'delta':>8s}   fork values (base | cand)")
for name in common:
