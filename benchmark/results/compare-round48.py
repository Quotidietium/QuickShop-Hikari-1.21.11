# R48 A/B comparison: round48-baseline (86f7ebcf1, R47 done) vs round48
# (click-create gate sweep). Median ns/op and median B/op across all
...
a_ns, b_ns = load("round48-baseline", "medianNsPerOp"), load("round48", "medianNsPerOp")
a_b, b_b = load("round48-baseline", "medianBytesPerOp"), load("round48", "medianBytesPerOp")

common = sorted(set(a_ns) & set(b_ns))
print(f"shared cases: {len(common)}\n")

print("== B/op axis (primary; near-deterministic) ==")
print(f"{'case':40s} {'baseline B/op':>15s} {'R48 B/op':>15s} {'delta':>8s}   fork values (base | cand)")
for name in common:
