#!/usr/bin/env python3
# R47 A/B comparison: round47-baseline (d929f30f2, R46 done) vs round47
# (lookup-chain tail sweep). Median ns/op and median B/op across all
...
a_ns, b_ns = load("round47-baseline", "medianNsPerOp"), load("round47", "medianNsPerOp")
a_b, b_b = load("round47-baseline", "medianBytesPerOp"), load("round47", "medianBytesPerOp")

common = sorted(set(a_ns) & set(b_ns))
print(f"shared cases: {len(common)}\n")

print("== B/op axis (primary; near-deterministic) ==")
print(f"{'case':40s} {'baseline B/op':>15s} {'R47 B/op':>15s} {'delta':>8s}   fork values (base | cand)")
for name in common:
