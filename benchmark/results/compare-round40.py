#!/usr/bin/env python3
# R40 A/B comparison: round40-baseline (f97083504, R39 done) vs round40
# (94d3b4e08, locate-chain cost stripping). Median ns/op and median B/op across all
...
a_ns, b_ns = load("round40-baseline", "medianNsPerOp"), load("round40", "medianNsPerOp")
a_b, b_b = load("round40-baseline", "medianBytesPerOp"), load("round40", "medianBytesPerOp")

common = sorted(set(a_ns) & set(b_ns))
print(f"shared cases: {len(common)}\n")

print("== B/op axis (primary; near-deterministic) ==")
print(f"{'case':40s} {'baseline B/op':>15s} {'R40 B/op':>15s} {'delta':>8s}   fork values (base | cand)")
for name in common:
