#!/usr/bin/env bash
# R50 A/B benchmark driver: baseline (1df3a5293, R48 completion point) vs candidate
# (hot-path event-construction gating: ExpiringSet deadline-map backing + dead
...
for f in 1 2 3; do
  ...
  (cd "$WT/benchmark" && mvn -q compile dependency:build-classpath -Dmdep.outputFile=target/cp.txt && \
    ...
    round50-baseline "R50 baseline $BASELINE (pre hot-path event-construction gating)")
  ...
  cp "$WT"/benchmark/results/round50-baseline-fork$f.json benchmark/results/
  ...
  (cd benchmark && mvn -q compile dependency:build-classpath -Dmdep.outputFile=target/cp.txt && \
    ...
    java -cp "$CP" -Dbenchmark.alloc="*" -Dbenchmark.fork=$f \
    com.ghostchu.quickshop.benchmark.BenchmarkMain \
    round50 "R50 candidate (hot-path event-construction gating)")
done
echo "ALL FORKS DONE"
