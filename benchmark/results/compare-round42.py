#!/usr/bin/env python3
# R42 A/B comparison: round42-baseline (cc7100e1c, R41 done) vs round42
# (192bb21b4, save-path item encode memo). Median ns/op and median B/op across all
...
a_ns, b_ns = load("round42-baseline", "medianNsPerOp"), load("round42", "medianNsPerOp")
a_b, b_b = load("round42-baseline", "medianBytesPerOp"), load("round42", "medianBytesPerOp")

common = sorted(set(a_ns) & set(b_ns))
print(f"shared cases: {len(common)}\n")

print("== B/op axis (primary; near-deterministic) ==")
print(f"{'case':40s} {'baseline B/op':>15s} {'R42 B/op':>15s} {'delta':>8s}   fork values (base | cand)")
for name in common:
