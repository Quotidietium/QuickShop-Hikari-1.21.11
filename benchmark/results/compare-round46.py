#!/usr/bin/env python3
# R46 A/B comparison: round46-baseline (ea77593a7, R45 done) vs round46
# (permission node and retrieve-event sweep). Median ns/op and median B/op across all
...
a_ns, b_ns = load("round46-baseline", "medianNsPerOp"), load("round46", "medianNsPerOp")
a_b, b_b = load("round46-baseline", "medianBytesPerOp"), load("round46", "medianBytesPerOp")

common = sorted(set(a_ns) & set(b_ns))
print(f"shared cases: {len(common)}\n")

print("== B/op axis (primary; near-deterministic) ==")
print(f"{'case':40s} {'baseline B/op':>15s} {'R46 B/op':>15s} {'delta':>8s}   fork values (base | cand)")
for name in common:
