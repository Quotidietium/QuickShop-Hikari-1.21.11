#!/usr/bin/env python3
# R41 A/B comparison: round41-baseline (b056e6c56, R40 done) vs round41
# (a59e5293a, menu item access costs). Median ns/op and median B/op across all
...
a_ns, b_ns = load("round41-baseline", "medianNsPerOp"), load("round41", "medianNsPerOp")
a_b, b_b = load("round41-baseline", "medianBytesPerOp"), load("round41", "medianBytesPerOp")

common = sorted(set(a_ns) & set(b_ns))
print(f"shared cases: {len(common)}\n")

print("== B/op axis (primary; near-deterministic) ==")
print(f"{'case':40s} {'baseline B/op':>15s} {'R41 B/op':>15s} {'delta':>8s}   fork values (base | cand)")
for name in common:
