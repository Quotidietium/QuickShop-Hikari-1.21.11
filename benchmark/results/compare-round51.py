# R51 A/B comparison: round51-baseline (214f9795f, R50 done) vs round51
# (click-entry measurement collapse: info-panel single snapshot/scan, listener-gated
# sign-render dedup, gate/cap scan sharing, transaction+operation item adoption). Median ns/op and median B/op across all
...
a_ns, b_ns = load("round51-baseline", "medianNsPerOp"), load("round51", "medianNsPerOp")
a_b, b_b = load("round51-baseline", "medianBytesPerOp"), load("round51", "medianBytesPerOp")

common = sorted(set(a_ns) & set(b_ns))
print(f"shared cases: {len(common)}\n")

print("== B/op axis (primary; near-deterministic) ==")
print(f"{'case':40s} {'baseline B/op':>15s} {'R51 B/op':>15s} {'delta':>8s}   fork values (base | cand)")
for name in common:
