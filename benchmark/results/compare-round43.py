#!/usr/bin/env python3
# R43 A/B comparison: round43-baseline (c2f2cfff6, R42 done) vs round43
# (e24a83b32, block-state snapshot gate sweep). Median ns/op and median B/op across all
...
a_ns, b_ns = load("round43-baseline", "medianNsPerOp"), load("round43", "medianNsPerOp")
a_b, b_b = load("round43-baseline", "medianBytesPerOp"), load("round43", "medianBytesPerOp")

common = sorted(set(a_ns) & set(b_ns))
print(f"shared cases: {len(common)}\n")

print("== B/op axis (primary; near-deterministic) ==")
print(f"{'case':40s} {'baseline B/op':>15s} {'R43 B/op':>15s} {'delta':>8s}   fork values (base | cand)")
for name in common:
