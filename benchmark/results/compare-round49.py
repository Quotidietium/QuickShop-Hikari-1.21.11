# R49 A/B comparison: round49-baseline (7810e0ec1, R48 done) vs round49
# (listener click-path residuals). Median ns/op and median B/op across all
...
a_ns, b_ns = load("round49-baseline", "medianNsPerOp"), load("round49", "medianNsPerOp")
a_b, b_b = load("round49-baseline", "medianBytesPerOp"), load("round49", "medianBytesPerOp")

common = sorted(set(a_ns) & set(b_ns))
print(f"shared cases: {len(common)}\n")

print("== B/op axis (primary; near-deterministic) ==")
print(f"{'case':40s} {'baseline B/op':>15s} {'R49 B/op':>15s} {'delta':>8s}   fork values (base | cand)")
for name in common:
