# R50 A/B comparison: round50-baseline (1df3a5293, R48 done) vs round50
# (hot-path event-construction gating). Median ns/op and median B/op across all
...
a_ns, b_ns = load("round50-baseline", "medianNsPerOp"), load("round50", "medianNsPerOp")
a_b, b_b = load("round50-baseline", "medianBytesPerOp"), load("round50", "medianBytesPerOp")

common = sorted(set(a_ns) & set(b_ns))
print(f"shared cases: {len(common)}\n")

print("== B/op axis (primary; near-deterministic) ==")
print(f"{'case':40s} {'baseline B/op':>15s} {'R50 B/op':>15s} {'delta':>8s}   fork values (base | cand)")
for name in common:
