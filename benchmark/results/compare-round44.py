#!/usr/bin/env python3
# R44 A/B comparison: round44-baseline (8e78643b3, R43 done) vs round44
# (economy provider-resolution sweep). Median ns/op and median B/op across all
...
a_ns, b_ns = load("round44-baseline", "medianNsPerOp"), load("round44", "medianNsPerOp")
a_b, b_b = load("round44-baseline", "medianBytesPerOp"), load("round44", "medianBytesPerOp")

common = sorted(set(a_ns) & set(b_ns))
print(f"shared cases: {len(common)}\n")

print("== B/op axis (primary; near-deterministic) ==")
print(f"{'case':40s} {'baseline B/op':>15s} {'R44 B/op':>15s} {'delta':>8s}   fork values (base | cand)")
for name in common:
