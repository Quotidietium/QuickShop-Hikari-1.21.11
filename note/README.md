...
| 代码规模 | quickshop-bukkit 190 用例全绿的裁剪后规模：api/bukkit/common/platform 主体 + addon 7 个启用 + compat 10 个启用（R37 裁剪后，见 [06-功能裁剪-R37.md](06-功能裁剪-R37.md)；betonquest/angelchest 维持停用） |
| 数据库 | MySQL 或 H2(MODE=MYSQL)，schema 版本 20，EasySQL + HikariCP |
| 测试 | quickshop-bukkit 190 用例全绿（R38–R41 新增 13 用例：分配轴改写等价、哈希契约/世界快照回归、定位链路由与菜单物品访问等价；浸泡用例因本机 IO 劣化暂超时，见 note/06；回归 + 查找表索引 + DB 写缓存 + 文本缓存 + QUser 驻留 + 事件快路径 + 匹配免克隆 + 迭代器快照 + 木牌调度 + 点击路径 + 交易观测 + 指标批处理 + 剩余 DB 写批处理 + 文本预解析等价 + 日志懒队列 + 展示物重送 + 菜单库存快照 + 开箱扫描快路径 + 签名渲染单扫 + 区块加载快门 + 签名排程去重 + 杂项热路径快照 + 价格格式化快照 + 监听/展示物配置快照 + 库存操作变更日志 + 扫描预闸快速计数） |
