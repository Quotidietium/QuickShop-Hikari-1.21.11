...
2. 逐条 `supplyAsync(workStealingPool)` 并行反序列化（DataRawDatabaseInfo：shopType/shopState/物品 platform.decodeStack 优先/permissions JSON/extra YAML）。owner/taxAccount 的 QUser 反序列化经 DESERIALIZED_CACHE 驻留（序列化串为键，5 分钟过期），万店几十主场景大幅免重复解析与异步用户名查找。
...
**脏标记持久化**：任一 setter 置 dirty → ShopDataSaveWatcher 每 5 分钟批量 update()；写库失败保留 dirty 下轮重试。2026-08-16 起 updateShop 带**不变跳写**（persistedRecordByShop 记录上次落库内容，未变直接返回）与**去重 SELECT 备忘录**（dataIdByRecord，免全字段扫描）；每笔交易的指标插入经 dataIdByShopId 缓存免 locateShopDataId SELECT（28μs→114ns）。三缓存 TTL 10 分钟，removeData/removeShop/purgeIsolated/recovery 导入处显式失效。
...
sequenceDiagram
    ...
    TS-->>SM: 成功 → 木牌更新入 SignUpdateWatcher（R10：≤0.5s 内合并刷新）
...
**扫描与观测（R15，2026-08-17）**：actionSelling/actionBuying 不再在交易前独立扫描箱/背包——
预检（previewBuy/SellToShop）单次测得箱库存/箱空间/交易者库存/交易者空间，经
`TradeResult.observation()`（`TradeObservation`）透出给失败消息与售罄/装满通知；
第三方 TradeService 空观测回退真实扫描。买方向箱计数处发 ShopInventoryCalculateEvent
保持 InternalListener 的 qs_external_cache 更新节奏。匹配器未命中路径类型门前置（异类槽免克隆）。
...
| SignUpdateWatcher | 10 tick | 队列刷木牌文本（单次限时 50ms）；交易成功后也走此队列（R10，条目携带交易者 locale，去重合并） |
